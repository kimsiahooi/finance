...
const handleDateSelect = (selectedDate: DateValue | undefined) => {
    if (selectedDate) {
        const currentHours = model.value
            ? new Date(model.value).getHours()
            : null;
        const currentMinutes = model.value
            ? new Date(model.value).getMinutes()
            : null;
...
const dateResult = computed(() =>
    model.value
        ? formatDateTime(model.value, 'YYYY-MM-DD hh:mm:ssa')
        : (props.placeholder ?? 'YYYY-MM-DD hh:mm:ssa'),
);
